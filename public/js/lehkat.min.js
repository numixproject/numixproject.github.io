...
*/
$(function(){
	$("body").append('<style id="ahmrxStyleSheet">.validateError{box-shadow: 1px -1px 15px #d64937!important;}</style>');
	$("form").submit(function(e){
		...
		th.find(".axBlank").each(function(){
			...
			if(val=="")
			{
				...
				{
					id=$(this).attr('id');
					$("label[for='"+id+"']").addClass('validateError');
				}
		...
		th.find(".axImg").each(function(){
			...
			patt=/^[a-zA-Z0-9-_\.]+\.(jpg|gif|png|jpeg|svg)$/g;
			if(!patt.test(val) && val!="" )
			{
				flag=false;
				$(this).addClass("validateError");
		...
		if(flag)
		{
			th.attr("data-allow","1")
		}
		else
		{
			th.attr("data-allow","0");	
		}
		return flag;
	});
})